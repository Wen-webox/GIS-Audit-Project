# 首席工程師多檔協同開發階段 (🔥 補丁與疊加模式 PATCH MODE)

請依照先前的規劃，為下列所有檔案撰寫完整的程式碼。
⚠️ 【嚴格補丁規範】：本次修改為「補丁」性質。你必須**完整保留**現有架構中的任何既有功能與邏輯，新需求必須是「疊加」或「優化」，絕對不可破壞、精簡或替換原有的核心系統！

⚠️ 【完整產出規範】：請遵守 MASTER_PROMPT 鐵律，產出完整的全量代碼，絕對禁止使用「...」或「原有程式碼保持不變」等佔位符。

## 檔案: ui\components\info_cards.py
```python
# -*- coding: utf-8 -*-
# Project: Environmental Audit GIS
# Module: info_cards.py
...
## 檔案: plugins\animator_3d.py
```python
# -*- coding: utf-8 -*-
# Project: Environmental Audit GIS
# Module: animator_3d.py


```

## 檔案: ui\file_control.py
```python
# -*- coding: utf-8 -*-
# Project: Environmental Audit GIS
# Module: file_control.py


```

## 檔案: ui\map_webview.py
```python
# -*- coding: utf-8 -*-
# Project: Environmental Audit GIS
# Module: map_webview.py


```

## 檔案: assets\www\leaflet_map.html
```html
<!-- Leaflet WebView Placeholder -->
<html><body><div id='map'></div></body></html>
```

## 檔案: assets\www\map_logic.js
```javascript

```

## 檔案: plugins\data_interpolator.py
```python
# -*- coding: utf-8 -*-
# Project: Environmental Audit GIS
# Module: data_interpolator.py
...
# NEW_FEATURE: 升級為 Kivy GIS 架構的入口模板，同時保留原有的防閃退攔截器
...
PROJECT_META_JSON = {
    ...
    "constraints": ["跨平台相容", "SOLID 原則", "Clean Architecture", "Kivy/KivyMD UI", "SQLite 離線防呆", "KML 批次解析"]
}
...
class AutoProjectManager:
    def __init__(self):
        ...
        # 2. 專案初始化器 (環保稽查圖台) 結構定義
        # NEW_FEATURE: 擴展結構以支援 KML 解析、SQLite、WebView 資源與 3D 動畫
        self.gis_structure = {
            "core": ["sqlite_manager.py", "kml_parser.py", "base_task.py", "__init__.py"],
            "plugins": ["api_poi_connector.py", "animator_3d.py", "data_interpolator.py", "__init__.py"],
            ...
            "assets/kml_samples": [],
            "assets/www": ["leaflet_map.html", "map_logic.js", "style.css"], # WebView 資源
    ...
    def print_menu(self):
        self.clear_screen()
        print("="*70)
        print(" 🚀 終極自動化專案管理器 V13.0 (Kivy GIS 環保稽查特化版)")
        print("="*70)
    ...
    def menu_1_env_setup(self):
        ...
        if not user_prompt.strip():
            domain = "⭐ [推薦] 環保稽查 GIS 專案 (Kivy + Leaflet WebView)"
        ...
        with open(meta_path, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "constraints": ["跨平台相容", "SOLID 原則", "Clean Architecture", "Windows/Android 路徑相容", "Kivy/Leaflet 架構"]
    ...
    def menu_2_initialize_gis_project(self):
                ...
                if not os.path.exists(file_path):
                    with open(file_path, 'w', encoding='utf-8') as f:
                        if file.endswith(".py"):
                            ...
                            f.write("<!-- Leaflet WebView Placeholder -->\n<html><body><div id='map'></div></body></html>")
        ...
        # NEW_FEATURE: 改為寫入 Kivy 與 GIS 解析所需套件
        if not os.path.exists(req_path) or True: # 強制更新依賴清單
            with open(req_path, 'w', encoding='utf-8') as f:
                f.write("kivy\nkivymd\nfastkml\nlxml\npandas\nrequests\n")
            print("✅ 建立 GIS 基礎 requirements.txt 完成 (包含 Kivy, fastkml 等)。")
    ...
    def menu_4_state_snapshot(self):
        print("\n--- 4. 專案狀態快照 ---")
        print("💡 [引導] 此功能會擷取專案目錄樹與程式碼骨架，可供 AI 了解專案全貌。")
    ...
    def menu_5_time_machine(self):
        print("\n--- 5. 時光回溯 (專案備份) ---")
        print("💡 [引導] 在執行 AI 大規模修改代碼前，為專案建立安全的還原點。")
    ...
    def menu_6_generate_master_prompt(self):
        ...
        print("💡 [引導] 產出給 AI 的最高開發鐵律，請在第一次對話時將其貼給 AI 閱讀。")
        # NEW_FEATURE: 全面更新 Prompt 以適應 Kivy GIS 架構
        prompt_content = f"""# 終極開發契約 MASTER_PROMPT (V13.0 Kivy GIS Edition)
...
"""
        ...
        print("MASTER_PROMPT.md 已成功生成，包含最新 Kivy GIS 防護鐵律。")
        input("\n按 Enter 鍵返回選單...")
    ...
    def menu_7_ai_guide(self):
        ...
        if choice == '1':
            goal = input("\n請輸入您的開發目標 (例如: 實作 KML 批次解析入 SQLite 邏輯): ")
    ...
    def _extract_ast_skeleton(self, filepath: str) -> str:
        """AST 骨架萃取：保留 class/def 簽名與 docstring，其餘轉為 pass"""
        try:
    ...
    def menu_8_memory_extraction(self):
        print("\n--- 8. 記憶提取與 AST 精準防爆框萃取 ---")
        print("💡 [引導] 當 AI 忘記專案脈絡，使用此功能強制提取特定檔案內容與全域骨架，重置 AI 記憶。")
    ...
    def menu_10_launch_radar(self):
        print("\n--- 10. 啟動雷達 (代碼品質與膨脹監控) ---")
        print("💡 [引導] 背景監控程式碼，若單檔或方法超標會主動警報。")
    ...
    def menu_11_android_build(self):
        ...
        print("💡 [引導] 在發布前檢查 Android APK 編譯屬性與靜默閃退攔截。")
        
        # 1. 自動資源掃描 (NEW_FEATURE: 擴充支援 Web 資源)
...
▶️ 功能 1 [環境與智慧領域建置]:
   - 使用時機: 全新專案剛開啟時。
   - 步驟: 執行後，系統會幫您建好虛擬環境 (.venv) 及隱藏檔。

▶️ 功能 2 [專案初始化器 (GIS 結構與依賴)]:
   - 使用時機: 需要開發環保稽查等 GIS 圖台時。
   - 步驟: 執行後會自動產生 Kivy GIS 分層架構 (含 SQLite/WebView)，
     並「自動幫您安裝(pip install)」Kivy, fastkml 等必備套件。

▶️ 功能 3 [套件管理]:
   - 使用時機: 當您在 Terminal B 透過 pip 安裝了新套件後。
   - 步驟: 執行此功能將自動把依賴寫入 requirements.txt。

【第二階段：代碼快照與智能協同 (AI Collaboration)】
▶️ 功能 4 & 5 [快照與時光機]:
   - 使用時機: 打算讓 AI 進行大幅度修改「前」的保險措施。
   - 步驟: 產出 AST 骨架幫助 AI 認識專案，並備份到 .time_machine。

▶️ 功能 6 [生成契約]:
   - 使用時機: 新開一個 ChatGPT / Claude 對話視窗時。
   - 步驟: 產出 MASTER_PROMPT.md (包含 KML處理與防呆鐵律)，直接貼給 AI 閱讀以規範其行為。

▶️ 功能 7 [AI 智慧嚮導 (Phase 1 & 2) ⭐核心功能⭐]:
   - 使用時機: 想要開發新功能 (例如實作 3D 車輛動畫) 時。
   - 步驟: 
     1. 選擇 Phase 1，輸入需求，將產出的提示詞貼給 AI，取得「檔案清單」。
     2. 選擇 Phase 2，將 AI 的回覆貼回終端機。系統會啟動「補丁模式」，
        抓取既有程式碼生成開發指令 (PHASE2_PROMPT.md)。
     3. 交給 AI，AI 即可在不破壞既有架構下完成疊加開發。

▶️ 功能 8 [記憶提取]:
   - 使用時機: AI 開始胡言亂語或忘記專案架構時。
   - 步驟: 輸入檔案名稱，系統產出 RESCUE_PROMPT.md 幫 AI 重置記憶。

【第三階段：監控與發布 (Monitor & Build)】
▶️ 功能 10 [啟動雷達]:
   - 使用時機: 專案日漸龐大時，放著讓它在背景掃描。

▶️ 功能 11 [打包 APK 優化]:
   - 使用時機: 準備發布到 Android 平板前。
   - 步驟: 系統將自動掃描 GIS 資源 (.kml, .html) 並注入防閃退機制。
==============================================================
"""
        print(manual)
        input("按 Enter 鍵返回選單...")
...
## 檔案: assets\www\style.css
```text

```

## 檔案: plugins\api_poi_connector.py
```python
# -*- coding: utf-8 -*-
# Project: Environmental Audit GIS
# Module: api_poi_connector.py


```

## 檔案: ui\components\vehicle_styles.py
```python
# -*- coding: utf-8 -*-
# Project: Environmental Audit GIS
# Module: vehicle_styles.py


```

## 檔案: GPS_Stoppoint.kml
```text
# 這是全新檔案，請生成完整代碼
